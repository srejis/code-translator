...
별도 런처 창 없이 브라우저의 코드 뷰어가 바로 열립니다. 실행 흐름은 다음과 같습니다.
...
선택하면 기존 기록을 즉시 사용합니다. 기록이 없으면 코드 뷰어 안에 `분석 시작`
버튼이 나타납니다. 최신 소스로 다시 만들려면 같은 화면의 `새로 분석` 버튼을
누릅니다. 새 분석이 실패해도 기존 정상 기록은
보존됩니다.

프로젝트 경로는 직접 입력하거나 운영체제 폴더 선택 대화상자로 고를 수 있습니다.
분석 중에는 현재 파일과 진행 개수가 코드 뷰어 안에 표시되며, 완료 후 새 탭을
열지 않고 같은 페이지에 파일 트리와 코드를 표시합니다. 사용자가 접은 파일 트리
폴더는 다른 파일을 선택하거나 검색·필터를 적용해도 현재 브라우저 세션 동안
접힌 상태를 유지합니다.
